...
bool ll_cam_dma_sizes(cam_obj_t *cam)
{
    ...
    if (cam->jpeg_mode) {
        /* ESP32-CAM live workloads (Wi-Fi + admin + Telegram) can fragment the
           internal DMA-capable heap enough that the default 32 KB JPEG DMA ring
           never fits. Keep the JPEG DMA ring intentionally small here so camera
           capture remains viable after boot on the 4 MB AI Thinker boards. */
        cam->dma_half_buffer_cnt = 3;
        cam->dma_node_buffer_size = 1024;
